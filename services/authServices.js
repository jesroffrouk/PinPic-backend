...
function createAuthServices({userRepository,helperRepository,jwt,bcrypt,crypto,generateVerifyTokens,generateUniqueUsername,logger}) {
    return {
      ...
      getUserProfile: async (userPublicId) => {
        ...
        const {id:userId} = (await helperRepository.getIdFromPublicId('users',userPublicId))?.rows[0]
