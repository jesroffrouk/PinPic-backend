...
// using custom error for now in but gonna move notification section to specific service
...
const placeControllers = {
  ...
  getImageById: catchAsync(async (req, res) => {
    ...
    res
      .status(201)
      .json(new ApiResponse(200, { post: result.data }, result.message));
  }),
  upvoteImage: catchAsync(async (req, res) => {
    ...
    const result = await metadataServices.upVoteImage(
      userid,
      imgid,
      react_type
    );
  ...
  setComment: catchAsync(async (req, res) => {
    ...
    const userId = req.user.id;
    //  upvote services logic
    const result = await metadataServices.setCommentToPost(
      userId,
      postId,
      comment
    );
  ...
  getComments: catchAsync(async (req, res) => {
    ...
    res
      .status(200)
      .json(new ApiResponse(200, { comments: result.data }, result.message));
  }),
  ...
  getNotification: catchAsync(async (req, res) => {
    ...
    const userResult = await placesModels.getIdFromPublicId(
      'users',
      userPublicId
    );
    ...
    res
      ...
      .json(
        new ApiResponse(
          ...
          'notifications retrieved successful'
        )
      );
  }),
  getLocationName: catchAsync(async (req, res) => {
    ...
    const longitude = req.query.longitude;
    // security: validate Inputs
    const result = await locationServices.getLocationName({
      latitude,
      longitude,
    });
    ...
    res
      .status(200)
      .json(new ApiResponse(200, { location: result.data }, result.message));
  }),
  // collections
  ...
  getCollection: catchAsync(async (req, res) => {
    ...
    // security: Validate Inputs
    const result = await metadataServices.getCollection(
      userId,
      createdAt,
      postId
    );
